...
当前插件 Host 所绑定组件的根树（全屏）；需要组合时，`parent_node(handle)` 把它挂到
...
- framework 仍规定一个 `DefaultXComponent` 对应一个 native module；插件契约只接收已经绑定好的
  Host/context，不接受 module、window 或 surface 路由参数。
- 同一个插件实例/component 可用不同 WebView ID 同时创建多个实例；每个实例拥有独立 mount key 和
  controller，同 ID recreate 才替换旧实例。ID 只在当前插件实例内唯一；ArkTS 会生成进程唯一的
  内部 ArkWeb tag，不同插件实例使用相同业务 ID 不会冲突。
...
在 `create` 前用 `WebviewCallbacksBuilder` 按 facade-local WebView ID 声明回调：
...
protocol 和 proxy 声明，供同 facade 后续 appearance 重建 controller。该兜底不依赖 closing 阶段还能
...
`WebviewProtocol::register` 的新声明只能在进程级 engine 初始化前调用。不同插件 facade 拥有
独立 Rust 状态，但共享 ArkWeb engine；第一个 WebView create 会让所有已激活的插件 facade
先 flush 声明，再初始化 engine。Ability 重建时重复相同 scheme + options 是幂等操作；engine 启动
后才激活的 facade 只能复用进程已注册且 options 相同的 scheme，不能再新增 custom scheme。ID handler 和 JS proxy 应优先在 `create`
