...
| `crates/plugin-clipboard` | `ohos.clipboard` — async image clipboard write |
| `crates/plugin-updater` | `ohos.updater` — async application update check/install |
| `crates/plugin-menu` | `ohos.menu` — async menu state plus typed click callbacks |
| `crates/plugin-statusbar` | `ohos.statusbar` — async desktop status-bar operations and callbacks |
| `crates/plugin-version` | `ohos.version` — sync platform version/capability queries |
